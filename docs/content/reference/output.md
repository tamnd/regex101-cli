...
describes what users get. Pick a format with `-o`, or let regex101 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
regex101 <command> -o table   # aligned columns for reading
regex101 <command> -o jsonl   # one JSON object per line, for piping
regex101 <command> -o json    # a single JSON array
regex101 <command> -o csv     # spreadsheet friendly
regex101 <command> -o tsv     # tab-separated
regex101 <command> -o url     # just the URL column
regex101 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
regex101 <command> --fields id,title,url
```
...
```bash
regex101 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
regex101 <command>            # a table, because this is a terminal
regex101 <command> | wc -l    # JSONL, because this is a pipe
```
